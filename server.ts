...
type Id = number
interface Item { id: Id; nombre: string; cantidad: number; precio?: number; categoria?: string }
...
async function leer(): Promise<Item[]> {
  try { return JSON.parse(await fs.readFile(file, "utf8")) as Item[] } catch { return [] }
}
...
app.post("/api/items", async (req, res) => {
  ...
  const categoria = body.categoria !== undefined ? String(body.categoria) : undefined
  if (!Number.isInteger(id) || id <= 0 || !nombre || !Number.isFinite(cantidad)) return res.status(400).json({ error: "datos" })
